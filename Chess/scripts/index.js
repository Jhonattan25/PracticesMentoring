...
En caso de que las filas y las columnas no sean numeros pares mayor de 7, el programa muestra una alerta indicando que se tuvo un error mostrando que pudo ser alguno de esos dos casos. */
function startGame() {
  let rows = parseInt(document.getElementById("rows").value);
  let columns = parseInt(document.getElementById("columns").value);
  turn = document.form.piece.value; //Obtener el valor del radio button
      ...
      for (let j = 0; j < columns; j++) {
        if (j >= columns / 2 - 4 && j <= columns / 2 + 3) {
          if (i === 0) {
            ...
            board[i].splice(0, 1);
            board[rows - i - 1] = board[rows - i - 1].concat(
              ",&#x2659;,&#x2659;".repeat(4).split(",")
            );
...
function paintChessBoard(board) {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    for (let j = 0; j < board[0].length; j++) {
      ...
      if (flag) elementColumn.classList.add("dark");
...
function selectCell(x, y) {
  ...
  if (
    ...
    (turn === "W" &&
      character.charCodeAt(0) > 51 &&
      character.charCodeAt(0) < 58)
  ) {
    if (selectedCell != null) {
      selectedCell.classList.remove("flicker");
    ...
    selectedCell.classList.add("flicker");
  } else {
    movePiece(x, y);
  }
}

//Funcion que sirve para mover la pieza seleccionada, se debe verificar que las filas o columnas a mover sean las permitidas segun la ficha seleccionada
function movePiece(x, y) {
  if (selectedCell != null && checkMovement(x, y)) {
    ...
    selectedCell.setAttribute(
      "onclick",
      `movePiece(${selectedCellX}, ${selectedCellY})`
    );
    selectedCell = null;
...
// Esta funcion verifica el tipo de ficha y hace el llamado a la funcion respectiva para verificar el movimiento
...
function checkMovementPawn(x, y) {
  ...
  if (chessBoard[selectedCellX][selectedCellY] === "&#x265F;") {
    if (
      ((selectedCellX === 1 && differenceX === 2) || differenceX === 1) &&
      differenceY === 0
    )
      return true;
  } else {
    if (
      ((selectedCellX === chessBoard.length - 2 && differenceX === -2) ||
        differenceX === -1) &&
      differenceY === 0
    )
      return true;
...
function checkMovementKnight(x, y) {
  ...
  if (
    (differenceX === 2 || differenceX === -2) &&
    (differenceY === 1 || differenceY === -1)
  )
    return true;

  if (
    (differenceY === 2 || differenceY === -2) &&
    (differenceX === 1 || differenceX === -1)
  )
    return true;
...
function checkMovementQueen(x, y) {
  if (checkMovementBishop(x, y) || checkMovementRook(x, y)) return true;
